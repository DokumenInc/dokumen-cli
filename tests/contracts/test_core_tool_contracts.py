...
from dokumen.scaffold import generate_scaffold, generate_test_scaffold, validate_scaffold
...
def test_removed_tools_are_not_public():
    ...
    assert "legacy_web_fetch" not in public_tools
    assert "anthropic_web_search" not in public_tools


def test_repo_has_no_git_provider_specific_references():
    forbidden = [
        "Git" + "Lab",
        "git" + "lab",
        "GIT" + "LAB",
    ...
    offenders = []
    for path, content in _tracked_text_files(Path(".")):
...
def test_repo_has_no_hosted_service_coupling():
    forbidden = [
        "DOKUMEN" + "_AGENT_ID",
        "DOKUMEN" + "_MCP_ENDPOINT",
        "DOKUMEN" + "_API_TOKEN",
        "api." + "dokumen.app",
        "auth/" + "validate",
        "agent_" + "loader",
    ...
    offenders = []
    for path, content in _tracked_text_files(Path(".")):
        build_key = "build-" + "back" + "end"
        if str(path) == "pyproject.toml" and build_key in content:
            content = content.replace(build_key, "")
        for term in forbidden:
            if term in content:
                offenders.append(f"{path}: {term}")
...
def test_generated_scaffolds_use_current_sop_shape():
    scaffold = yaml.safe_load(generate_scaffold("sops/refund-escalation-sop.md"))
    validation = validate_scaffold(scaffold)

    assert validation.valid
    assert scaffold["files"] == [{"path": "sops/refund-escalation-sop.md"}]
    assert scaffold["executor"]["tools"] == ["read_file"]
    assert "list_files" not in yaml.safe_dump(scaffold)
    assert "dokumen add" not in yaml.safe_dump(scaffold)


def test_generated_action_scaffold_is_valid():
    scaffold = yaml.safe_load(
        generate_test_scaffold(
            "sops/refund-escalation-sop.md",
            action="Review the ticket and recommend the next action.",
            assertion="Executor identifies whether Finance escalation is required.",
        )
    )
    validation = validate_scaffold(scaffold)
    ...
    assert scaffold["files"] == [{"path": "sops/refund-escalation-sop.md"}]
    assert scaffold["executor"]["tools"] == ["read_file"]
    assert scaffold["judges"][0]["include_executor_output"] is True
...
def test_distribution_metadata_shares_only_the_public_cli_surface():
    ...
    sentry_package = "sentry" + "-sdk"
    assert "onepassword-sdk" not in dependencies
    assert sentry_package not in dependencies
    assert "onepassword-sdk>=0.3.0" in project["optional-dependencies"]["integrations"]
    assert sentry_package not in "\n".join(project["optional-dependencies"]["integrations"])
...
def test_manifest_includes_docs_examples_and_packaged_authoring_skill():
    ...
    assert "recursive-include examples/business-sop *.md *.yaml" in manifest
