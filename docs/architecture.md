...
- A project configuration defines providers, models, coverage, tools, memory,
  coordinator behavior, and execution defaults.
...
- Browser setup starts Playwright MCP when browser tools are required.
- Setup runs pre-test commands or background processes.
- Explore discovers relevant files and injects retrieval context.
- Executor runs the main agent, or coordinator mode decomposes the work across
  multiple workers.
...
run so browsers, setup processes, and temporary resources are closed.
