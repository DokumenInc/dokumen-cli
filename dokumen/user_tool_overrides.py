...
code for each tool in the standalone CLI.
"""
...
# Valid systems for user overrides.
VALID_OVERRIDE_SYSTEMS = frozenset({"test", "explore"})
...
IMPLEMENTABLE_IN: dict[str, frozenset[str]] = {
    # CLI read-only
    "read_file": frozenset({"test", "explore"}),
    "list_files": frozenset({"test", "explore"}),
    "search_files": frozenset({"test", "explore"}),
    "glob_files": frozenset({"test", "explore"}),
    # CLI write tools
    "write_file": frozenset({"test"}),
    ...
    # Explore-only alias
    "explore_code": frozenset({"explore"}),
}
