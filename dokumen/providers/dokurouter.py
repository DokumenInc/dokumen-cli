...
class DokuRouter(Provider):
    ...
    usage:
        # explicit provider
        p = DokuRouter(model="gpt-4o", provider_name="openai", api_key="sk-...")

        # inferred from model name
        p = DokuRouter(model="openai/gpt-4o")

        # custom endpoint (any openai-compatible API)
        p = DokuRouter(model="my-model", api_base="http://localhost:8080/v1", api_key="test")
    """
