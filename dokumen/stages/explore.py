...
class ExploreStage(PipelineStage):
    ...
    async def run(self, ctx: PipelineContext) -> PipelineContext:
        ...
        if not should_explore:
            logger.info("stage.explore.skip", test_id=ctx.test_id, reason="explore disabled")
            return ctx

        logger.info("stage.explore.start", test_id=ctx.test_id, required_files=ctx.files)
        if is_debug():
        ...
        if explore_result:
            ...
            ctx.explore_cache_creation_tokens = getattr(explore_result, "cache_creation_tokens", 0)
            ctx.explore_cache_read_tokens = getattr(explore_result, "cache_read_tokens", 0)
            ctx.explore_model = getattr(explore_result, "model", None)

            logger.info(
                "stage.explore.result",
                test_id=ctx.test_id,
        ...
        if ctx.files and explore_result:
            ...
            if missing_files:
                ...
                if is_debug():
                    debug(
                        f"[EXPLORE STAGE] Running deterministic check for "
                        f"{len(missing_files)} missing file(s)"
                    )
                still_missing = self._check_files_on_disk(ctx, missing_files, explore_result)

                if still_missing:
                    logger.warning(
                        "stage.explore.missing_files",
                        test_id=ctx.test_id,
                        missing_files=still_missing,
                    )
                    ctx.fail(self._format_missing_files_error(ctx, still_missing, explore_result))
                    ctx.explore_status = "fail"
                    ...
                    logger.info(
                        "stage.explore.all_files_recovered",
                        test_id=ctx.test_id,
        ...
        if explore_result and (
            ...
            logger.info(
                "stage.explore.context_injected",
                test_id=ctx.test_id,
                success=explore_result.success,
    ...
    async def _run_explore(self, ctx: PipelineContext):
        ...
        return await explore_agent.explore(goal=goal, on_progress=ctx.on_explore_event)
    ...
    def _check_files_on_disk(self, ctx, missing_files, explore_result) -> List[str]:
        ...
        for file_path in missing_files:
            ...
            if os.path.isfile(normalized):
                logger.info(
                    "stage.explore.deterministic_recovery", test_id=ctx.test_id, file_path=file_path
                )
    ...
    def _inject_explore_context(self, ctx, explore_result) -> None:
        ...
        if context_block:
            ctx.executor.user_prompt = f"{context_block}\n\n---\n\n{ctx.executor.user_prompt}"
