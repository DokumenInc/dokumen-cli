"""
Config command for dokumen.yaml.

dokumen config          # Open interactive config editor (tabbed: status / config)
...
SCHEMA: List[Tuple[str, List[Tuple[str, str, Any, str]]]] = [
    ...
    (
        "skills",
        [
            ("enabled", "bool", True, "reusable instruction injection"),
            ("dir", "choice:skills/,./skills,", "skills/", "custom instruction folder"),
            ("include_system", "bool", True, "include bundled reusable instructions"),
            (
                "max_skills_per_prompt",
                "choice:5,10,15,20",
                10,
                "instructions injected per agent turn",
            ),
        ],
    ),
    (
