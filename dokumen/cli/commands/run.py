...
from ..helpers import (
    EXIT_SUCCESS, EXIT_FAILURE, EXIT_CONFIG_ERROR, EXIT_INVALID_ARGS,
    load_config, filter_tests, run_async, validate_folder_path
)
from ..formatters import (
    results_to_dict, results_to_junit, results_to_tap,
    print_results_text, print_run_settings, make_progress_callback, make_tool_call_callback,
    make_conversation_callback, make_executor_complete_callback, make_judge_complete_callback
)
...
@click.option('--folder', '-d', default=None, help='Run tests in folder and subfolders. Use "." for root-level only.')
@click.option('--timeout', '-t', type=int, help='Override timeout (seconds)')
@click.option('--bail', '-b', is_flag=True, help='Stop on first failure')
@click.option('--force', '-f', is_flag=True, envvar='DOKUMEN_FORCE', help='Force run skipped/cached tests')
@click.option('--dry-run', is_flag=True, help='Show tests without running')
@click.option('--output', '-o', type=click.Choice(['text', 'json', 'junit', 'tap']), default='text',
              help='Output format')
...
@click.pass_context
def run(ctx, tests: Tuple[str], grep: Optional[str], for_file: Optional[str],
    ...
    """
    from ...loader import load_all_scaffolds, get_configured_provider, get_configured_providers
...
async def _run_tests(tests, timeout=None, bail=False, use_cache=True,
                     config=None, quiet=False, verbose=False, parallel=None):
    """Async test runner implementation."""
    from ...test_suite import TestSuite, TestSuiteConfig
    from ...loader import get_configured_provider
    from ..formatters import (
        make_progress_callback, make_tool_call_callback,
        make_conversation_callback, make_executor_complete_callback, make_judge_complete_callback
    )
