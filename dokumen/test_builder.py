...
def create_provider(name: str, api_key: str = None, model: str = None, **kwargs):
    ...
    if name == "anthropic" and not kwargs.get("force_direct_provider"):
        from .providers.anthropic import AnthropicProvider

        return AnthropicProvider(api_key=api_key, model=model)

    # Non-Anthropic direct-provider adapters call external provider APIs directly.
    from .providers.direct_provider import DirectProviderRouter

    return DirectProviderRouter(
        model=model,
        provider_name=name,
        api_key=api_key,
        ...
        **{
            ...
            if k not in ("api_base", "base_url", "force_direct_provider", "enable_thinking")
        },
    )
